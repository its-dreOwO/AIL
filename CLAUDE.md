...
- **Phase 2 — one architecture change (~24h retrain, upside only).** Lever **chosen by the supervisor: a gated FFN** (superseding the Phase 0/1-informed candidate, iterative multi-person refinement, which is NOT being pursued).

**Phase 0+1 DONE (2026-07-04). Results in `docs/results/humof-improve/`.** Phase 0: reconstructed `primary_filterC` gates 61.6% of target windows on D (13,162/34,370) → absolute numbers not Table-1-comparable. Phase 1 (deterministic A/B, same subset): **TTA4 test-time rotation-averaging wins path 107.15 (−2.57mm) / pose 66.27 (−0.54mm)** vs det baseline 109.71/66.81, retraining-free + exactly reproducible; checkpoint avg{50,60,70} smaller (−0.35/−0.25). Fixed eval nondeterminism (unseeded DataLoader-worker numpy, ~0.03mm scene-resampling noise) via `worker_init_fn`.

### Phase 2 — gated FFN (COMPLETE 2026-07-16 — null result: path −0.48mm vs original, t=−1.19, ns; rebuild-fidelity yardstick −0.14mm. See `docs/results/humof-improve/phase2-gated-ffn.md`)

Supervisor directive: replace the transformer-layer FFN with a **gated FFN**; report the numbers. Spec: `docs/superpowers/specs/2026-07-15-humof-gated-ffn-design.md`. Full state: `docs/results/humof-improve/HANDOFF.md`.

- **⚠️ The old VM `hik-simlpe-train` was DELETED** (no snapshot) — it destroyed the filters, the preprocess script, and all Phase 1 code, which existed nowhere else. Everything was rebuilt from the Phase 0/1 write-ups. **All Phase 2 work therefore lives in `humof_repro/` in THIS repo, never only on compute** — incl. `patches/phase2-gated-ffn.patch`, which is the only record of the HUMOF-side edits (HUMOF is a separate checkout).
- **Change:** param-matched **SwiGLU** (`hidden = round(2*ff_dim/3)`) in `models/transformers.py`, behind env flag `HUMOF_GATED_FFN` so both arms run byte-identical code. Verified exactly: baseline logged `10.01099M` params, gated logs `10.01377M` = **+2,780**, precisely the predicted +0.028%. Caveat to report: SwiGLU bundles the gate *and* ReLU→SiLU; a delta can't be split between them.
- **Prior expectation: small, possibly unmeasurable.** `dim_q=60` vs `ff_dim=2048` is a **34× expansion ratio** — gating buys least exactly there. A null result honestly reported is the correct outcome.
- **Arms:** gated on **Modal A100** (`humof_repro/modal_app.py`, ~7.8 min/epoch ≈ 9h) vs. the existing 109.71; baseline re-run on **GCP T4** (~24h, off critical path) as a pipeline-fidelity check, since comparing to 109.71 changes 5 things at once (FFN, GPU, python, `hik` source, filter reconstruction).
- **⚠️ Modal preempts containers** and re-runs the function from the top → training restarted at epoch 0. `modal_app.py::train` autodetects the newest checkpoint → `HUMOF_CP_ITER`, using `main.py`'s existing *integer* resume path. **Do NOT use main.py's own `'auto'` branch** — it `int()`s the env var before it can match `'auto'`, then blocks on `input()`. Save interval 10→2 via `HUMOF_SAVE_INTERVAL`.
- **Modal volumes only persist on explicit `commit()`** — a crash at hour 8 loses everything without the periodic committer thread in `train()`.
- **`modal app stop` needs `-y`** (no TTY) — a silently-failed stop once left **two A100s running at $4.20/hr**. Always verify exactly one app is alive after launching.

- **VM access: there is currently NO GCP VM** — `hik-humof-base` was deliberately deleted 2026-07-16 after Phase 2 completed (all artifacts rescued first). To provision a new T4: `humof_repro/gcp_grab_t4.sh` (cycles all 51 T4 zones — capacity, not quota, is the blocker on a credit project) then `gcp_setup_baseline.sh`. The `hikvm` ssh alias (ControlMaster multiplexing, ~0.4s/cmd) is dangling until a new VM exists; refresh its `HostName` from `gcloud compute instances describe <name> --zone=<zone> --format='value(networkInterfaces[0].accessConfigs[0].natIP)'`.
- **Dataset:** local copy at `/home/dre/Downloads/hik_dataset/data` (3.9GB) — rsync `poses`+`scenes`+`body_models` to a fresh VM at `~/hik_dataset/` (~2-6 MB/s, ~20-30 min). The Modal volumes (`humof-data`, `humof-ckpt`) were **deleted 2026-07-16** with the rest of the Modal teardown; the local copy is the only one.
- **⚠️ conda `defaults` channel now blocks non-interactive installs** (`CondaToSNonInteractiveError` — new since the original VM). `gcp_setup_baseline.sh` pins `--override-channels -c conda-forge` to sidestep it without accepting Anaconda's ToS; torch/numpy come from pip wheels so numerics are unaffected.
...
- **Config edits in `~/HUMOF`:** `conf0.py` `DATASET_name='hik'` (ships with `'humanise'` UNcommented last — last assignment wins, so the repo as-shipped does not run hik); `conf2.py` `CUDA_VISIBLE_DEVICES='0'` (ships as `'1'` — no GPU on a 1-GPU VM!), `num_workers=8`. All now env-driven; see `humof_repro/patches/phase2-gated-ffn.patch`.
- **Run:** `setsid bash ~/run_train.sh > ~/train_hik.log`. ~20 min/epoch on T4 (~7.8 on an A100), GPU-bound, ~24 h for 70 ep. Ckpts → `checkpoints/hik-releaseV0.1/`; eval → `results/hik-releaseV0.1/err.csv`.
- **torch 1.13.0+cu117 supports up to sm_86** → H100 (sm_90) is unusable without changing torch (which would change numerics). **A100 is the ceiling.**
