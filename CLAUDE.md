...
### Phase 2 — gated FFN (IN FLIGHT 2026-07-15, deadline Fri 2026-07-17)
...
- **VM access:** `ssh hikvm` (direct alias, key `~/.ssh/google_compute_engine`, ControlMaster multiplexing → ~0.4s/cmd vs ~12s via `gcloud compute ssh`). Current VM is **`hik-humof-base`, zone `us-west4-b`**. ⚠️ External IP changes on every stop/start — refresh `HostName` in `~/.ssh/config` from `gcloud compute instances describe hik-humof-base --zone=us-west4-b --format='value(networkInterfaces[0].accessConfigs[0].natIP)'`. **T4 capacity is scarce on a credit project** (quota is 1 GPU but every zone can be out of stock at once) — `humof_repro/gcp_grab_t4.sh` cycles all 51 T4 zones until one succeeds.
